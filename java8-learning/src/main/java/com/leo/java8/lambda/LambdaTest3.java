package com.leo.java8.lambda;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.function.Consumer;
import java.util.function.Function;
import java.util.function.Predicate;
import java.util.function.Supplier;

import org.junit.Test;

/**
 * Java8 内置的四大核心函数式接口
 * <pre>
 * Consumer<T> : 消费型接口
 *     void accept(T t);
 *
 * Supplier<T> : 供给型接口
 *     T get();
 *
 * Function<T, R> : 函数型接口
 *     R apply(T t);
 *
 * Predicate<T> : 断言型接口
 *     boolean test(T t);
 * </pre>
 *
 * @since 2019-3-8
 */
public class LambdaTest3 {

    // Consumer<T> 消费型接口：无返回值
    @Test
    public void test1() {
        happy(10000, (m) -> System.out.println("双十一剁手：" + m + "元"));
    }
    private void happy(double money, Consumer<Double> con) {
        con.accept(money);
    }

    // Supplier<T> 供给型接口：产生对象
    @Test
    public void test2() {
        // 10 个 0~100 的随机数
        List<Integer> numList = getNumList(10, () -> (int) (Math.random() * 101));

        for (Integer num : numList) {
            System.out.println(num);
        }
    }
    // 产生指定个数的整数，并放入集合中
    private List<Integer> getNumList(int num, Supplier<Integer> sup) {
        List<Integer> list = new ArrayList<>();

        for (int i = 0; i < num; i++) {
            list.add( sup.get() );
        }

        return list;
    }

    // Function<T, R> 函数型接口：处理对象
    @Test
    public void test3() {
        String newStr = strHandler("\t\t\t 修剪一下   ", (str) -> str.trim());
        System.out.println(newStr);

        String subStr = strHandler("修剪一下", (str) -> str.substring(1, 4));
        System.out.println(subStr);
    }
    // 用于处理字符串
    private String strHandler(String str, Function<String, String> fun) {
        return fun.apply(str);
    }

    // Predicate<T> 断言型接口：返回真假
    @Test
    public void test4() {
        List<String> list = Arrays.asList("Hello", "justZero", "Lambda", "www", "ok");
        List<String> strList = filterStr(list, (s) -> s.length() > 3);

        for (String str : strList) {
            System.out.println(str);
        }
    }
    // 将满足条件的字符串，放入集合中
    private List<String> filterStr(List<String> list, Predicate<String> pre) {
        List<String> strList = new ArrayList<>();

        for (String str : list) {
            if (pre.test(str)) {
                strList.add(str);
            }
        }

        return strList;
    }
}
